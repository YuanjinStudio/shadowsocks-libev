...
int tls12_ticket_pack_auth_data(tls12_ticket_auth_global_data *global, server_info *server, uint8_t *outdata) {
    int out_size = 32;
    time_t t = time(NULL);
//    outdata[0] = (char) (t >> 24);
//    outdata[1] = (char) (t >> 16);
...
//    ss_sha1_hmac_with_key(hash, outdata, out_size - OBFS_HMAC_SHA1_LEN, key, (int) (server->key_len + 32));
    ss_sha1_hmac_with_key(hash, (char *)outdata, out_size - OBFS_HMAC_SHA1_LEN, key, (int) (server->key_len + 32));
    free(key);
...
//            int outlength = 0;
//            int len;
           out_buffer = (uint8_t *) malloc((size_t) (datalength + 4096));
            size_t start = 0;
            size_t outlength = 0;
            size_t len;
...
//        ss_sha1_hmac_with_key(hash, out_buffer, (int) (pdata - out_buffer), key, (int) (self->server.key_len + 32));
        ss_sha1_hmac_with_key(hash, (char *)out_buffer, (int)(pdata - out_buffer), key, (int)(self->server.key_len + 32));
        free(key);
