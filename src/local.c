...
int
create_and_bind(const char *addr, const char *port) //绑定用户发送句柄
{
...
server_recv_cb(EV_P_ ev_io *w, int revents)
{
    ...
    if (r == 0) {
        ...
        if (errno == EAGAIN || errno == EWOULDBLOCK) {
            ...
            printf("----------------ERROR:server_recv_cb_recv");
            close_and_free_remote(EV_A_ remote);
            close_and_free_server(EV_A_ server);
            return;
        }
                ...
                if (vpn) {
                    ...
                    if (!not_protect) {
                        if (protect_socket(remote->fd) == -1) {
                            ...
                            printf("----------------ERROR:连接失败protect_socket");
                            close_and_free_remote(EV_A_ remote);
                            close_and_free_server(EV_A_ server);
                            return;
                        }
                    ...
                    if (s == -1) {
                        if (errno == CONNECT_IN_PROGRESS) {
                            // in progress, wait until connected
                             printf("----------------in progress, wait until connected");
                            remote->buf->idx = 0;
                            ev_io_stop(EV_A_ & server_recv_ctx->io);
                            ev_io_start(EV_A_ & remote->send_ctx->io);
                            return;
...
//    cork_dllist_add(&all_connections, &server->entries_all);
    cork_dllist_add(&all_connections, &server->entries);
...
static void
close_and_free_server(EV_P_ server_t *server) // 关闭ss和释放
{
    if (server != NULL) {
...
static char *obfs_ssr;
static char *obfs_param;
static char *protocol_param;
ss_server_t serv_cfg;
server_def_t *serv;
struct sockaddr_storage *storage;
...
{
    ssfile = profile.SSStatusFile; // ss 状态文件目录
    writeFile(0);
    ...
    protocol   = profile.protocol; // SSR
    obfs_ssr = profile.obfs; // SSR
    obfs_param  = profile.obfs_param; // SSR
    protocol_param = profile.protocol_param; // SSR
...
//    int remote_num = 1;
    listen_ctx.fd = 0;
    serv_cfg.protocol = protocol;
    serv_cfg.obfs = obfs_ssr;
    serv_cfg.obfs_param = obfs_param;
    serv_cfg.protocol_param = protocol_param;
//    serv->protocol_global = malloc(sizeof(void *) * remote_num);
//    serv->obfs_global = malloc(sizeof(void *) * remote_num);
//    memset(serv->protocol_global, 0, sizeof(void *) * remote_num);
//    memset(serv->obfs_global, 0, sizeof(void *) * remote_num);
    // SSR end
    
    if (mode != UDP_ONLY) {
        // Setup socket
        int listenfd;
        listenfd = create_and_bind(local_addr, local_port_str); //一开始是本地的local_addr 127.0.0.1 端口 0   监听的用户句柄
        if (listenfd == -1) {
            ERROR("ss绑定失败：bind()"); //绑定失败
            return -1;
        }
        if (listen(listenfd, SOMAXCONN) == -1) { // socket listen 监听
            ERROR("ss监听失败：listen()"); //监听失败
            return -1;
        }
        setnonblocking(listenfd);
...
//    cork_dllist_init(&inactive_profiles);舍弃
    cork_dllist_init(&all_connections);
    
    printf("--------strat中ss的端口---%d",sssock_port(listen_ctx.fd));
    
    
    cb(listen_ctx.fd, data); // listen_ctx.fd == 11
    // Enter the loop
    ev_run(loop, 0);
...
sssock_port (int fd) { //fd 用户句柄
    ...
    if (getsockname(fd, (struct sockaddr *)&sin, &len) < 0) { //getsockname 來獲取本地和連接端的IP和Port，其實通常Client端都會指定Server端的IP和Port，然後再去連接，也不用去獲取了
        return 0;
    }else{
        return ntohs(sin.sin_port);// ntohs()是一个函数名，作用是将一个16位数由网络字节顺序转换为主机字节顺序[1]  。
    }
}
...
//重新开启SS
int
Restart_ss_local_server(profile_t profile){
   ssfile = profile.SSStatusFile; // ss 状态文件目录
    writeFile(0);
    
    sprintf(remote_port_str, "%d",profile.remote_port);
    if (get_sockaddr(profile.remote_host, remote_port_str, storage, 0, ipv6first) == -1) {
        return -1;
    }
    serv->addr = serv->addr_udp = storage;
    serv->addr_len = serv->addr_udp_len = (int) get_sockaddr_len((struct sockaddr *) storage);
    
    enc_init(&serv->cipher, profile.password, profile.method);
    init_obfs(serv, profile.protocol, profile.protocol_param, profile.obfs, profile.obfs_param);
    return 0;
}
