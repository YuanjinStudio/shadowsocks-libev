...
static const int supported_ciphers_iv_size[CIPHER_NUM] = {
    0,  0,  0,  6, 16, 16, 16, 16, 16, 16, 16,  8, 16, 16, 16,  8,  8,  8,  8, 16,  8,  8, 12,12
};

static const int supported_ciphers_key_size[CIPHER_NUM] = {
    16,  16, 16, 16, 16, 16, 24, 32, 16, 24, 32, 16, 16, 24, 32, 16,  8, 16, 16, 16, 32, 32, 32,32
};
...
//            return crypto_aead_chacha20poly1305_encrypt(c, m, NULL, m, mlen,
//                                                              ad, adlen, n, k);;
            break;
    }
    // always return 0
...
int
ss_decrypt(cipher_env_t* env, buffer_t *cipher, enc_ctx_t *ctx, size_t capacity) //解码
{
    if (ctx != NULL) {
        static buffer_t tmp = { 0, 0, 0, NULL };
...
enc_key_init(cipher_env_t *env, int method, const char *pass)
{
    ...
    if (method == SALSA20 || method == CHACHA20 || method == CHACHA20IETF || method == CHACHA20POLY1305IETF) {
#if defined(USE_CRYPTO_OPENSSL)
        cipher.info    = NULL;
